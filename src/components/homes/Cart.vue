  ...
  export default {
    ...
    methods: {
      ...
      getTotalPrice () {
        ...
        return balance.reduce((sum, item) => sum + item.price * item.quantity, 0)//reduce()数组方法，第一个参数累加，第二个初始值
      },
      onCheckChanged () {
      ...
      payAll () {
        ...
        this.checkedArray.forEach((item, index) => {
          item ? this.order.push(this.cart[index]) : inCart.push(this.cart[index])})
        this.cart = inCart
        this.setCheckedArray()
        this.setStore()
      }
